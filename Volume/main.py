...
detector = htm.HandRecognizer(detectionCon=0.7)
...
while True:
    ...
    img = detector.IdentifyHands(img)
    FingersvalueList = detector.GetPosition(img, draw=False)
    ...
    fps = 1 / (cTime - pTime)
    pTime = cTime
    cv2.putText(img, f'FPS: {int(fps)}', (40, 50), cv2.FONT_ITALIC, 1, (255, 0, 255), 3)
