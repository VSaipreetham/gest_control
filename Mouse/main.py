...
recognizer = ref.HandRecognizer(maxHands=1)
...
while status:
    ...
    if len(FingersList) != 0:
        x1, y1 = FingersList[8][1:]
        x2, y2 = FingersList[12][1:]
        # print(x1, y1, x2, y2)

        # 3. Check which fingers are up
        raisefingers = recognizer.RaiseFingers()
        # print(fingers)
        cv2.rectangle(image, (redFrame, redFrame), (camWidth - redFrame, camHeight - redFrame),
                      (0, 255, 204), 2)
        # 4. Only Index Finger : Moving Mode
        if raisefingers[1] == 1 and raisefingers[2] == 0:
        ...
        # 8. If both the index and middle fingers are up then it is in the Clicking Mode
        if raisefingers[1] == 1 and raisefingers[2] == 1:
            # 9. Find distance between fingers
            length, image, lineInfo = recognizer.GapFinding(8, 12, image)
            print(length)
